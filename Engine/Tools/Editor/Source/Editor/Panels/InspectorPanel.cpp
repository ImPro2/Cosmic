...
namespace Cosmic
{
    ...
    void InspectorPanel::RenderComponents(Entity entity)
    {
        ...
        RenderComponent<NativeScriptComponent>("Native Script Component", entity, [&](NativeScriptComponent& component)
		{
			...
			{
				auto tooltipCallback = [field]()
				{
					ImGui::BeginTooltip();
                    ImGui::TextUnformatted(field->GetTypeName().c_str());
					ImGui::EndTooltip();
				};

				ImGuiUtils::SetTooltipCallback(tooltipCallback);
            ...
            ImGuiUtils::SetTooltipCallback(nullptr);
		});
    }
